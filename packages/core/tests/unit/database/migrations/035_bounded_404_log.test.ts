...
describe("035_bounded_404_log migration", () => {
	...
	it("dedups on retry when only `hits` was added before the previous attempt crashed", async () => {
		await sql`
			INSERT INTO _emdash_404_log (path, created_at) VALUES
				('/a', '2026-01-01T00:00:00Z'),
				('/a', '2026-01-02T00:00:00Z')
		...
		// Different partial-failure shape from the regression case above:
		// `hits` is present but `last_seen_at` never made it. The migration
		// must still add the missing column, dedup, and create the unique
		// index without skipping any of those steps.
		await db.schema
			.alterTable("_emdash_404_log")
			.addColumn("hits", "integer", (col) => col.notNull().defaultTo(1))
			.execute();
		...
		const rows = await sql<{ path: string; hits: number }>`
			SELECT path, hits FROM _emdash_404_log
		`.execute(db);
		expect(rows.rows).toEqual([{ path: "/a", hits: 2 }]);
	});

	it("is idempotent on full success (running twice in a row)", async () => {
		await sql`
			INSERT INTO _emdash_404_log (path, created_at) VALUES
				('/a', '2026-01-01T00:00:00Z'),
				('/a', '2026-01-02T00:00:00Z')
